...
1. **Swift API Design Guidelinesに従う**
   - https://www.swift.org/documentation/api-design-guidelines/ を遵守
   - 明確で自己文書化された名前を使用
   - 簡潔さより明確さを優先

2. **不必要な冗長性を避ける**
   - コード、コメント、コミットメッセージに絵文字を使用しない
   - コメントは簡潔に、「何を」ではなく「なぜ」に焦点を当てる
   - 宣伝的な言葉遣いを避ける

### 実装とレビューの考慮事項

1. **型安全性**
   - 生の値よりenumを優先（例: `IndexType` enum vs 生の `String`）
   - Swiftの型システムを使用して無効な状態を防ぐ
   - 可能な場合、コンパイラにプロトコル準拠を合成させる（Equatable、Hashable、Codable）

2. **制御フロー**
   - 早期リターンと事前条件検証には `guard` を使用
   - 早期リターンのない単純な条件ロジックには `if` を使用
   - 不必要なネストを避ける

3. **パフォーマンス**
   - 不必要な配列アロケーションを避ける（Collectionジェネリクス、スライス、`withUnsafeBytes`を使用）
   - タスクに適切なデータ構造を使用
   - 大きなシーケンスには遅延評価を検討

4. **API設計**
   - 確立されたFoundationDB命名規則に従う（例: Tupleエンコーディングに `pack`/`unpack`）
   - 適切な場合、他の言語バインディングとの一貫性を維持
   - 名前空間の汚染を避ける（標準ライブラリ型を拡張するのではなく、適切な型で静的メソッドを使用）

5. **エラーハンドリング**
   - エラーコードを持つ構造体ベースのエラーを使用（SwiftNIOパターン）
   - 明確で実行可能なエラーメッセージを提供
   - APIドキュメントでエラーケースを文書化

6. **並行性パターン**
   - このプロジェクトでは `final class: Sendable` + `Mutex` パターンを使用（actorではない）
   - `DatabaseProtocol` には `nonisolated(unsafe)` を使用
   - 可変状態は `Mutex<State>` で保護
   - ロックスコープは最小限に保つ（I/Oを含めない）

### セルフレビューチェックリスト

PRを提出する前に確認：

- [ ] GitHub Issueが存在し、`Fixes #123` 構文を使用してPR説明にリンクされている
- [ ] PRのサイズが適切（テストを除いて約500行）
- [ ] コードがSwift API Design Guidelinesに従っている
- [ ] コード/コメント/コミットに絵文字や宣伝的な内容がない
- [ ] 可能な場合、コンパイラ合成された準拠を使用
- [ ] 生の値より型安全なenumを優先
- [ ] パフォーマンスの考慮事項に対処（不必要なアロケーションを避ける）
- [ ] すべてのテストが合格（`swift test`）
- [ ] すべてのコード変更に対して手動レビューを実施
