...
public struct RecordableMacro: MemberMacro, ExtensionMacro {
    ...
    public static func expansion(
        ...
        let indexInfo = extractIndexInfo(from: members)
    ...
    /// Extract index information from #Index and #Unique macro calls
    ///
    /// This method scans the struct body for #Index/#Unique macro expansion expressions
    /// and extracts:
    /// - Type name from generic parameter
    /// - Field names from KeyPath expressions
    /// - Custom name from 'name:' parameter
    /// - Unique flag
    private static func extractIndexInfo(
        ...
        for member in members {
            // Look for macro expansion expressions (#Index or #Unique)
            guard let exprStmt = member.decl.as(ExpressionStmtSyntax.self),
                  let macroExpr = exprStmt.expression.as(MacroExpansionExprSyntax.self) else {
                continue
            }

            let macroName = macroExpr.macroName.text
            ...
            guard let genericArgs = macroExpr.genericArgumentClause,
            ...
            // Extract arguments
            let arguments = macroExpr.arguments
            ...
            // Process each KeyPath array argument (variadic support)
            // Example: #Index<User>([\.email], [\.username]) creates two separate indexes
            // Example: #Index<User>([\.country, \.city], name: "location_index") creates one compound index
